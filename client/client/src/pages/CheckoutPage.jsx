...
const CheckoutPage = () => {
  ...
  const validateData = (value) => {
    ...
    if (!value.firstName) {
      ...
      errors.firstName = "first name must be aleast of 4  character or long";
        ...
        if (paymentMethod === "cod") {
          try {
            let { data } = await axios.post(
              `${import.meta.env.VITE_APP_API_URL}/order/create`,
              {
                items: itemsToSend,
                shipping: {
                  ...checkoutData,
          ...
          try {
            let { data } = await axios.post(
              `${import.meta.env.VITE_APP_API_URL}/stripe/create-checkout-session`,
              {
                items: itemsToSend,
                shipping: {
                  ...checkoutData,
        ...
        <Box>
          ...
          <Box mt={5}>
            <Grid container>
                  ...
                  <Grid container mt={3}>
                    <Grid item xs={12}>
                      ...
                      >
                        ...
                        >
                          {City.getCitiesOfState("IN", checkoutData.state).map(
                            (state) => (
              ...
              <Grid item xs={12} md={6} lg={5} px={4} sx={{
                mt: {
                  xs: 2,
                  sm: 4,
                  md: 6
